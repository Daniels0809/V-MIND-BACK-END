...
hola, estoy en la realizacion de un proyecto que se basara en una pagina web gamificada sobre aprendizaje en areas de conocimiento, la web generara con IA un roadmap de aprendizaje basado en un formulario de caracterización que se la hara al usuario al momento de hacer un registro, en el roadmap generado sera por niveles los cuales se iran desbloqueando con la XP que te dan al terminar cada clase o cada modulo, entonces a mi me toco el rol de crear la base de datos que es la siguiente:
CREATE DATABASE IF NOT EXISTS crisalida;
USE crisalida;
CREATE TABLE users (
    user_id CHAR(36) PRIMARY KEY DEFAULT (UUID()),
...
teniendo esto en cuenta ahora me asignaron la siguiente feature: Gestión de Roadmaps & Módulos (Back B, Front E). Te hare envio de los product backlogs que me asignaron con las task que debo realizar, te lo enviare uno a uno despues de terminar cada una.
primer product backlog es: Creación de roadmaps personalizados, te enviare la primer task sobre este product backlog que es:
Create Roadmap Model & Migration: Create Sequelize model Roadmap in src/models/roadmap.model.js with fields:

id, title, description, difficulty, xp_reward, createdAt, updatedAt.
Migration in src/database/migrations/xxxx-create-roadmap.js.
